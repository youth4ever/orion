...
class PEcrypt:
    ...
    def __init__(self, aKey):
        """
        Initialise the class with the key that is used to encrypt/decrypt strings
        """
        self.key = aKey
...
# So we can't do : >>>   'hello' ^ 'world'
# Solution:   Fortunately, Python has the (amazing) itertools module:
...
# = izip(a, b) returns an iterator that aggregates elements from each of the iterables.
# It stops on the shortest input sequence (here, our message, since cycle(key) is infinite
