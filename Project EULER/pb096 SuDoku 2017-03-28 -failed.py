...
def update_tracker(G, tracker) :
    '''Updates the INITIAL tracker list which will be used to explore
    all possibilties. The tracker list becomes larger by exp(2)
    Therefore a list of 10 pairs will yield 2**10 possibilities
    '''
#     print(tracker,'\n\n')
...
def matrix_back_to_pos(pos, A, M):
    ''' A - initial matrix, M - completed to position matrix,
    pos- last position
    '''
    i, j = str(pos); i = int(i) ; j=int(j)
...
def get_position_choices(M, x , y):
    ''':Description: Important function which get the *FILL STATUS* of the matrix.
    :param M:   the matrix to be tested
    :return:    {'71': [1, 2, 3, 5]}, [] = if there is no available choice,
...
for i in range(10):     print(str(i+1)+'. ', previous_position(i), end=' ;    ')
...
def sudoku_main_solve( M, grid_nr) :
    Tracker, cnt = {}, 1
    pos, tracker_pos = 11, 11           # Starting position, Backtracking position
    BT ={}      # The Back Tracking counter, counts how many times the BT has been to a position
    A = deepcopy(M)
    # for b in range(100000) :
    while 1 :
        x, y = str(pos) ; x = int(x) ; y = int(y)

        ###  CASE 0 - This skip the positions which are already filled ! - applies when the pos is already filled
        if A[x-1][y-1] != 0 :
            while  A[x-1][y-1] != 0 :
                print('\nskipped by default :\t', pos, M)
                if pos == 99 :
                    show_complete(M, grid_nr )
                    return int(''.join([str(i) for i in M[0][:3]]))
        ...
        print('\n-- get_pos  -->\t\t', 'pos = ',  pos, '     choices = ', g )#, '     \nM= ', M,'\n')

        ### CASE 1 - Fill the matrix if 1 available choice - CASE 1
        if type(g) == list and  len(g) == 1 :
            M[x-1][y-1] = g[0]
            if pos == 99 :
                show_complete(M, 17)
                return int(''.join([str(i) for i in M[0][:3]]))

            if pos in Tracker : del(Tracker[pos])           # Here we delete the multiple choices from BT & Tracker if on
            if pos in BT : del(BT[pos])                     #    he preceedings runs were seen as multiple choices and Now there is  choice

            pos = next_position(pos)


        if type(g) == list and len(g) > 1 :         # Update the Tracker
            if pos not in Tracker : Tracker[pos] = g

        ### CASE 2 - Fill the matrix  IF Multiple Choices - CASE 2
        if pos in Tracker :
            M[x-1][y-1] = Tracker[pos][0]
            tracker_pos = pos
            if tracker_pos not in BT: BT[tracker_pos] = 1           # Create Back tracking rotation
            print(  '^^ FM        pos=', pos, Tracker[pos], '      Tracker=',Tracker, '       \nM=', M,'\n', '     cnt=', cnt, '   pos=', pos, '    BT=',BT )
            pos = next_position(pos)


        ### CASE 3 -  Start Backtracking ####
        if type(g) == list and  len(g) == 0 :
            pos = tracker_pos
            if BT[tracker_pos] != len(Tracker[tracker_pos]) :           #  First Rotation of the position list choices
                Tracker[tracker_pos] = rotate( Tracker[tracker_pos] , -1)
                M = matrix_back_to_pos(tracker_pos, A, M)
                BT[tracker_pos] +=1
                print( ' ** BM         pos=', pos ,Tracker[pos], '      Tracker=',Tracker, '       \nM=', M,'\n', '     cnt=', cnt, '   pos=', pos, '    BT=',BT )
                continue

            if BT[tracker_pos] == len(Tracker[tracker_pos]) :           # If the rotation is complete and we still arrive at the position we must go

                pos = previous_position(tracker_pos)                                       # back another tracker position
                # print('special assistant : ', pos,  [k for k,v in BT.items() if BT[k] != len(Tracker[k])] ,  [k for k,v in BT.items() if BT[k] != len(Tracker[k]) and k < pos ] )
                lst = [k for k,v in BT.items() if BT[k] != len(Tracker[k]) and k < pos ]
                if lst :  pos = max([k for k,v in BT.items() if BT[k] != len(Tracker[k]) and k < pos ])
                else : pos = pos



                for k,v in BT.items() :            # Here we reset all the Rotation counters for the elements bigger than the previous_pos
                    if k > pos  : BT[k] = 1

                for i in [k for k,v in BT.items() if k > pos ] :        # Here we delete all the Tracker & BT positions after going to the previous pos
                    del(BT[i])
                    del(Tracker[i] )
...
@2017-03-28, 17:00          ! THIS IS SAD VERY SAD ONLY ! After weeks of desperetly trying to solve this problem
I simply cannot ! If I solve  one of the grids, I will have another which FAILS ! My solution is too DAMN complicated !
I have no time to loose with this problem ! Sorry ! It doesn't worth it  ! Life is too SHORT !
